...
def Test(dataset, Recmodel, epoch, cold=False, w=None):
    ...
    with torch.no_grad():
        ...
        # print(results)
        print(f"Test results for Top-{world.topks}:")
        for idx, K in enumerate(world.topks):
            print(f"Top-{K} Precision: {results['precision'][idx]:.4f}")
            print(f"Top-{K} Recall: {results['recall'][idx]:.4f}")
            print(f"Top-{K} NDCG: {results['ndcg'][idx]:.4f}")
